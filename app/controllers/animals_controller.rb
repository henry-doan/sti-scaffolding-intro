class AnimalsController < ApplicationController
  ...
  def index
  	@animals = params[:type] ? @zoo.animals.where(race: race_class) : @zoo.animals
  end
  ...
  def create
  	...
  	if @animal.save
  		redirect_to sti_animal_path(@zoo), success: 'Animal created'
  	else
  		render :new
  	end
  end
  ...
  def update
  	if @animal.update(animal_params)
  		redirect_to sti_animal_path(@zoo, @animal.race, @animal)
  	else
  		render :edit
  	end
  end
